...
namespace DataReviewer.Task
{
    class TaskManager
    {
        ...
        public void ChangeTasksStatus(string taskName,string status,string comment="")
        {
            ...
            if (rw != null)
            {
                ...
                else if (status == TaskManager.MANUALCHECK_STATUS)
                {
                    ...
                    rw.set_Value(task_tbl.FindField("Comment"), comment);
                }
                rw.Store();
        ...
        public void SetGridPassed(string taskName, string gridcode)
        {
            IFeatureWorkspace ws = Util.ServerWorkspace as IFeatureWorkspace;
            IWorkspaceEdit wse = ws as IWorkspaceEdit;
            wse.StartEditing(true);
            ...
            ITable task_fc = ws.OpenTable("TaskGridLog");
            IQueryFilter filter = new QueryFilterClass();
            filter.WhereClause = "TaskName = '" + taskName + "' and " + "gridcode='"+gridcode+"'";
            ICursor cur = task_fc.Update(filter, false);
            IRow task_fea = cur.NextRow();
            if (task_fea != null)
            {
                task_fea.set_Value(task_fc.FindField("passed"), 1);
                cur.UpdateRow(task_fea);
            }
            wse.StopEditOperation();
            wse.StopEditing(true);
        }
