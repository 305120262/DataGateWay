...
{
    public class CheckLineEndPointsHasPoint : BaseChecker, IChecker
    {
        /// <summary>
        /// 线的端点上是否有点检查
        /// </summary>
        /// <param name="strParams">参数数组包含：线图层名称、点图层名称</param>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "线的端点上是否有点检查所需参数为空！请输入正确参数！";
                ...
                {
                    Message = "线的端点上是否有点检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：线的端点上没有点现象！";

                CheckErrorList = CheckerUtil.CheckLinesHasPoints(layerName, layerName2, strSQL, strSQL2, true, esriSpatialRelEnum.esriSpatialRelIntersects, "T********", strErrorMsg);
                
                return true;
            }
            catch (Exception err)
            {
                Message = "线的端点上是否有点检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
