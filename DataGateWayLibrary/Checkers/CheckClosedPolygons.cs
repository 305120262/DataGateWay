...
{
    public class CheckClosedPolygons : BaseChecker, IChecker
    {
        /// <summary>
        /// 面闭合检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "面闭合检查所需图层名为空！请输入正确图层名参数！";
                ...
                CheckErrorList = CheckerUtil.CheckClosedFeaturesInLayer(layerName, strSQL);

                return true;
            }
            catch (Exception err)
            {
                Message = "面闭合检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
