...
{
    public class CheckContourLinesIntersect : BaseChecker, IChecker
    {
        /// <summary>
        /// 等高线相交检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "等高线相交检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：等高线相交现象！";

                CheckErrorList = CheckerUtil.CheckOverlapFeaturesInLayer(layerName, strSQL, esriSpatialRelEnum.esriSpatialRelCrosses, "T*T******", strErrorMsg);

                return true ;
            }
            catch(Exception err)
            {
                Message = "等高线相交检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
