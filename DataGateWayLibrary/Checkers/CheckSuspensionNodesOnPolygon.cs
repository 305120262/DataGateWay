...
{
    public  class CheckSuspensionNodesOnPolygon : BaseChecker, IChecker
    {
        /// <summary>
        /// 面悬挂检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "面悬挂检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：面悬挂现象！";

                CheckErrorList = CheckerUtil.CheckSuspensionNodesOnPolygon(layerName, layerName2, strSQL, strSQL2, esriSpatialRelEnum.esriSpatialRelTouches, "FT*******", strErrorMsg);
                
                return true;
            }
            catch (Exception err)
            {
                Message = "面悬挂检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
