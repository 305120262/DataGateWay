...
{
    public class CheckIntersectPolygons : BaseChecker, IChecker
    {
        /// <summary>
        /// 面叠盖检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "面叠盖相交检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：面叠盖现象！";

                CheckErrorList = CheckerUtil.CheckOverlapFeaturesInLayers(layerName, layerName2, strSQL, strSQL2, 34, esriSpatialRelEnum.esriSpatialRelOverlaps, "T*T***T**", strErrorMsg);
              
                return true;
            }
            catch (Exception err)
            {
                Message = "面叠盖检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
