...
{
    public class CheckTwoRedundantLines : BaseChecker, IChecker
    {
        /// <summary>
        /// 两个结合间的冗余线检查(不同编码的两条线之间是否存在重复线段)
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                CheckErrorList = CheckerUtil.CheckRedundantFeaturesInLayer(layerName, layerName2, fieldName, strSQL, strSQL2, false, esriSpatialRelEnum.esriSpatialRelOverlaps, "TT*T*****");
               
                return true;
            }
            catch (Exception err)
            {
                Message = "两个结合间的冗余线检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
