...
{
    public class CheckPolygonTouchLine : BaseChecker, IChecker
    {
        /// <summary>
        /// 面的边界是否被线压盖检查
        /// </summary>
        /// <param name="strParams">参数数组包含：面图层名称、线图层名称</param>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "面的边界是否被线压盖检查所需参数为空！请输入正确参数！";
                ...
                {
                    Message = "面的边界是否被线压盖检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：面的边界没有被线压盖现象！";

                CheckErrorList = CheckerUtil.CheckOverlapFeaturesInLayers(layerName, layerName2, strSQL, strSQL2, 32, esriSpatialRelEnum.esriSpatialRelTouches, "FF*FT****", strErrorMsg);
              
                return true;
            }
            catch (Exception err)
            {
                Message = "面的边界是否被线压盖检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
