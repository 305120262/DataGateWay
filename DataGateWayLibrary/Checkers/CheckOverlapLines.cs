...
{
    public class CheckOverlapLines : BaseChecker, IChecker
    {
        /// <summary>
        /// 检查图层中的重叠线
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "重叠线检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：重叠线现象！";

                CheckErrorList = CheckerUtil.CheckOverlapFeaturesInLayer(layerName, strSQL, esriSpatialRelEnum.esriSpatialRelContains, "TT*FFT***", strErrorMsg);
                
                return true;
            }
            catch (Exception err)
            {
                Message = "重叠线检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
