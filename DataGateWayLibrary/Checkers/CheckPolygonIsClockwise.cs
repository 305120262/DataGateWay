...
{
    public class CheckPolygonIsClockwise : BaseChecker, IChecker
    {
        /// <summary>
        /// 面顺时针检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "面顺时针检查所需图层名为空！请输入正确图层名参数！";
                ...
                CheckErrorList = CheckerUtil.CheckPolygonIsClockwise(layerName, strSQL, true);
               
                return true;
            }
            catch (Exception err)
            {
                Message = "面顺时针检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
