...
{
    public class CheckBrokenLines : BaseChecker, IChecker
    {
        /// <summary>
        /// 检查图层中的碎线
        /// </summary>
        /// <param name="paramsValue">参数数组包含：图层名称和指定的线长度</param>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "碎线检查所需图层名为空！请输入正确图层名参数！";
                ...
                {
                    CheckErrorList = CheckerUtil.CheckBrokenFeaturesInLayer(layerName, strSQL, "line");
                }
                else
                {
                    double dLen = Convert.ToDouble(strValue);
                    CheckErrorList = CheckerUtil.CheckBrokenFeaturesInLayer(layerName, strSQL, "line", dLen);
                }
                return true;
            }
            catch(Exception err)
            {
                Message = "碎线检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
