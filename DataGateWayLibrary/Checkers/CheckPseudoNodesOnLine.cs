...
{
    public class CheckPseudoNodesOnLine : BaseChecker, IChecker
    {
        /// <summary>
        /// 线伪结点检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "线伪结点检查所需图层名为空！请输入正确图层名参数！";
                ...
                string strErrorMsg = "错误描述：线伪结点现象！";

                CheckErrorList = CheckerUtil.CheckPseudoNodesLines(layerName, layerName2, strSQL, strSQL2, esriSpatialRelEnum.esriSpatialRelTouches, "FT*******", strErrorMsg);
              
                return true;
            }
            catch (Exception err)
            {
                Message = "线伪结点检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
