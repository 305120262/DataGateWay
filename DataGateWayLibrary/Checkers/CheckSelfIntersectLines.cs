...
{
    public class CheckSelfIntersectLines : BaseChecker, IChecker
    {
        /// <summary>
        /// 线自相交检查
        /// </summary>
        ...
        public bool CheckData()
        {
            try
            {
                ...
                {
                    Message = "线自相交检查所需图层名为空！请输入正确图层名参数！";
                ...
                CheckErrorList = CheckerUtil.CheckSelfIntersectFeaturesInLayer(layerName, strSQL);
                
                return true;
            }
            catch (Exception err)
            {
                Message = "线自相交检查出现异常，错误原因：" + err.Message;
                return false;
            }
        }
    }
